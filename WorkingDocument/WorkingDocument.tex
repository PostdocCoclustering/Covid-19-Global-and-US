...
\begin{abstract}
	\noindent In response to the global coronavirus pandemic, we study co-clustering of multivariate functional data as a way to simultaneously cluster both the rows and columns of the data matrix. We consider both US data and global data and show how homogeneous blocks of clusters gathering both specific time periods and states/countries are retrieved through the proposed methodology. The information prevailed by such methods are in a second stage plugged into the SIR epidemiological forecast model.  
